...
class MomForge:
    ...
    def birth_new_skeleton(self, num_components: int = 3):
        ...
        unique_id = str(uuid.uuid4())[:8]  # z. B. "a1b2c3d4"
        ...
        factsheet = {
            ...
            "facts": {
                "nodes": 0,  # später gefüllt
                "edges": 0,
                "density": 0.0,
                "modularity": 0.0,
                "feedback_loops": 0,
                "dominant_type": max(mixture, key=mixture.get)
            }
        }
        ...
        # Nach Generierung Factsheet füllen
        factsheet["facts"]["nodes"] = G.number_of_nodes()
        factsheet["facts"]["edges"] = G.number_of_edges()
        factsheet["facts"]["density"] = G.number_of_edges() / (
                    G.number_of_nodes() * (G.number_of_nodes() - 1)) if G.number_of_nodes() > 1 else 0
        factsheet["facts"]["modularity"] = nx.community.modularity(G, nx.community.greedy_modularity_communities(
            G)) if G.number_of_nodes() > 10 else 0.3
...
if __name__ == "__main__":
    ...
    # Automatisches Laden alter Skelette
    ...
    # Zentrale ancestry.json laden (falls vorhanden)
    ...
    survivors = []
    for _ in range(3):  # z. B. 3 neue Skelette pro Run
